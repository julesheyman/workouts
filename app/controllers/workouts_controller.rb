class WorkoutsController < ApplicationController
  before_action :current_user_must_be_workout_user, only: [:edit, :update, :destroy] 
  ...
  def index
    ...
    @workouts = @q.result(:distinct => true).includes(:user, :fastest_times, :sport).page(params[:page]).per(10)
  end
  ...
  def current_user_must_be_workout_user
    ...
    def workout_params
      params.require(:workout).permit(:description, :user_id, :sport_id, :distance, :date)
    end
end
